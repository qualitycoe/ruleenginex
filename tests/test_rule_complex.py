...
class TestRuleComplex(unittest.TestCase):
    def test_rule_evaluates_nested_dicts(self):
        rule = Rule(target="body", prop="user.details.age", op="equals", value=30)
    ...
    def test_rule_handles_empty_values(self):
        rule = Rule(target="body", prop="username", op="NULL", value=None)
    ...
    def test_rule_handles_varied_data_types(self):
        rule1 = Rule(target="body", prop="count", op="equals", value=5)
        rule2 = Rule(target="body", prop="is_active", op="equals", value=True)
        rule3 = Rule(target="body", prop="roles", op="ARRAY_INCLUDES", value="admin")
    ...
    def test_rule_handles_partial_data(self):
        rule = Rule(target="body", prop="settings.theme", op="equals", value="dark")
    ...
    def test_rule_jsonpath_array_evaluation(self):
        rule = Rule(target="body", prop="$.users[*].name", op="ARRAY_INCLUDES", value="Alice")
    ...
    def test_rule_empty_property_uses_entire_target(self):
        rule = Rule(target="body", prop=None, op="VALID_JSON_SCHEMA", value={"type": "object"})
    ...
    def test_rule_empty_string_property(self):
        rule = Rule(target="body", prop="", op="VALID_JSON_SCHEMA", value={"type": "object"})
        request_data = {"body": {"user": "Alice", "age": 30}}
        assert rule.evaluate(request_data) is True  # The entire body should match schema
